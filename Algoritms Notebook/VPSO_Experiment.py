...
def main(topology, dimension):    
    ...
    for benchmark_func in regular_functions:
        ...
        for simulation_id in range(num_exec):
            ...
            bests_iter, bests_eval = run_experiments(num_iterations, max_evaluations, part_size, func, 
                                                     search_space_initializer, topology=topology, min_ai=min_ai, 
                                                     vol_final=step_volitive_final, vol_init=step_volitive_init)
            end = time.time()
...
def run_experiments(n_iter, max_evaluations, part_size,  objective_function, search_space_initializer, topology, 
                    min_ai, vol_final, vol_init):
    
    opt1 = VPSO(objective_function=objective_function, space_initializer=search_space_initializer,
                n_iter=n_iter, max_evaluations = max_evaluations, part_size=part_size, inertia=0.73, cognitive=2.05,
                social=2.05, vel_max=10, clerck=True, topology = topology, auto_coef=True, min_w=min_ai, 
                vol_final=vol_final, vol_init=vol_init)
