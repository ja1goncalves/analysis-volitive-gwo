...
class VolitivePack2(object):
    ...
    def sniffing(self):
        ...
        for wolf in self.pack:
            if self.max_delta_fitness:
                wolf.aromatic_intensity = wolf.aromatic_intensity + \
                    (wolf.delta_fitness / self.max_delta_fitness)
    ...
    def gwo_movement(self):  # GWO Movement
        for wolf in self.pack:
            ...
            d_alpha = abs(self.c1 * self.alpha.pos + direction * wolf.pos)
            x1 = self.alpha.pos + direction * self.a1 * d_alpha

            self.update_variables()
            d_beta = abs(self.c1 * self.beta.pos + direction * wolf.pos)
            x2 = self.beta.pos + direction * self.a1 * d_beta

            self.update_variables()
            d_delta = abs(self.c1 * self.delta.pos + direction * wolf.pos)
            x3 = self.delta.pos + direction * self.a1 * d_delta
