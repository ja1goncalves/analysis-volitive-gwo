...
def main():
  dimension = 30
  print (f"starting EGWO ({dimension})")
  ...
  f_handle_csv = open(result_path + "/EGWO_iter.csv", 'w+')
  writer_csv = csv.writer(f_handle_csv, delimiter=",")
  header = ['opt', 'func', 'exec_time'] + [f"run{str(i+1)}" for i in range(num_iterations)]
  writer_csv.writerow(header)
  name_file = f'EGWO_dim_{dimension}_agents_{pack_size}_iter_{num_iterations}_eval_{max_evaluations}'
  ...
  for benchmark_func in regular_functions:
    ...
    for simulation_id in range(num_exec):
        ...
        row_csv = ['EGWO', func.function_name, (end - start)] + [b for b in fit_convergence[:num_iterations]]
